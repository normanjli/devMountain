...
// const changeImg = id => axios.put(`${baseURL}/${id}`).then(housesCallback).catch(errCallback)
const changeIMG = (id) => {
    let img= document.getElementById(`change${id}`).value
    axios.put(`${baseURL}/img/${id}`, {img})
    .then(housesCallback)
    .catch(errCallback)
}
...
function createHouseCard(house) {
    ...
    <input type = "text" id = "change${house.id}" placeholder = "enter new image url"></input>
    <button onclick = "changeIMG(${house.id})">Change IMG</button>
